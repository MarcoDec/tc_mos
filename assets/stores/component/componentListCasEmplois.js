...
export const useComponentListCasEmploisStore = defineStore('customerListCasEmplois', {
    actions: {
        setIdComponent(id){
            this.componentID = id
        },
        ...
        async deleted(payload) {
            ...
            this.customerCasEmplois = this.customerCasEmplois.filter(retard => Number(retard['@id'].match(/\d+/)[0]) !== payload)
        },
        async fetch() {
            ...
            this.customerCasEmplois = await this.updatePagination(response)
        },
    ...
    getters: {
        itemsComponentCasEmplois: state => state.customerCasEmplois.map(item => {
