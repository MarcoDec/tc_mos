...
export const useSocietyListCustomerStore = defineStore('societyListCustomer', {
    actions: {
        setIdSociety(id){
            this.societyID = id
        },
        // async addSocietyCustomer(payload){
        //     const violations = []
        //     try {
        ...
        async deleted(payload) {
            await api(`/api/customers/${payload}`, 'DELETE')
            this.societyCustomer = this.societyCustomer.filter(retard => Number(retard['@id'].match(/\d+/)[0]) !== payload)
        },
        async fetch() {
            ...
            const response = await api(`/api/customers?society.id=${this.societyID}`, 'GET')
            this.societyCustomer = await this.updatePagination(response)
        },
        async filterBy(payload) {
            let url = `/api/customers?society.id=${this.societyID}&`
            if (payload === ''){
                await this.fetch()
            } else {
                if (payload.name !== '') {
        ...
        async paginationSortableOrFilterItems(payload) {
            ...
            if (payload.filter.value === true && payload.sortable.value === true){
                ...
                response = await api(`/api/customers?society.id=${this.societyID}&page=${payload.nPage}`, 'GET')
                this.societyCustomer = await this.updatePagination(response)
            } else {
                if (payload.trierAlpha.value.composant === 'component') {
                    response = await api(`/api/customers?society.id=${this.societyID}&order%5B${payload.trierAlpha.value.composant}%5D=${payload.trierAlpha.value.trier.value}&page=${payload.nPage}`, 'GET')
                } else {
                    response = await api(`/api/customers?society.id=${this.societyID}&order%5Baddress.${payload.trierAlpha.value.composant}%5D=${payload.trierAlpha.value.trier.value}&page=${payload.nPage}`, 'GET')
                }
                this.societyCustomer = await this.updatePagination(response)
            }
        },
        ...
        async sortableItems(payload, filterBy, filter) {
            ...
            if (filter.value === true){
                ...
                if (payload.composant === 'component') {
                    response = await api(`/api/customers?society.id=${this.societyID}&order%5B${payload.composant}%5D=${payload.trier.value}&page=${this.currentPage}`, 'GET')
                } else {
                    response = await api(`/api/customers?society.id=${this.societyID}&order%5B${payload.produit}%5D=${payload.trier.value}&page=${this.currentPage}`, 'GET')
                }
                this.societyCustomer = await this.updatePagination(response)
            }
        },
    ...
    getters: {
        itemsSocietyCustomer: state => state.societyCustomer.map(item => {
            const newObject = {
                '@id': item['@id'],
                name: item.name,
