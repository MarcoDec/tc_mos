<script lang="ts" setup>
    ...
    const normalizedChart = useNamespacedGetters('needs', [
        'normalizedChart'
    ]).normalizedChart
</script>

<template>
    <div class="card" :list="list" :product-id="productId">
        <div class="no-gutters row">
            <div class="canvas col-sm-5">
                <Vue3ChartJs
                    :id="normalizedChart(productId).id"
                    :type="normalizedChart(productId).type"
                    :data="normalizedChart(productId).data"
                    :options="normalizedChart(productId).options"/>
            </div>
            <div class="col-sm-7">
                <div class="card-body">
                    <h5 class="card-title">
