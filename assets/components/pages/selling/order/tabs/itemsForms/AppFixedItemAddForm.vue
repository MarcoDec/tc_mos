...
async function updateFixedLocalData(value, localFixedData) {
    ...
    if (value.product && value.product !== initialLocalData.product) {
        ...
        await api(value.product, 'GET').then(async response => {
            measure.setQuantityToMinDelivery(localFixedData.value, response)
            await measure.getAndSetProductPrice(response, props.customer, props.order, localFixedData.value.requestedQuantity, localFixedData, fixedFormKey)
        })
        return
    }
    if (value.component && value.component !== initialLocalData.component) {
        ...
        await api(value.component, 'GET').then(async response => {
            measure.setQuantityToUnit(localFixedData.value, response)
            await measure.getAndSetComponentPrice(response, props.customer, props.order, localFixedData.value.requestedQuantity, localFixedData, fixedFormKey)
        })
        return
    }
    if (value.confirmedQuantity.value && value.confirmedQuantity.value !== initialLocalData.confirmedQuantity.value) {
        ...
        if (value.product) {
            await measure.getAndSetProductPrice(value.product, props.customer, props.order, localFixedData.value.confirmedQuantity, localFixedData, fixedFormKey)
            return
        }
        if (value.component) {
            await measure.getAndSetComponentPrice(value.component, props.customer, props.order, localFixedData.value.confirmedQuantity, localFixedData, fixedFormKey)
            return
        }
    }
    if (value.requestedQuantity.value && value.requestedQuantity.value !== initialLocalData.requestedQuantity.value) {
        // Si un produit a été sélectionné on récupère le prix unitaire associé au produit et au client
        if (value.product) {
            await measure.getAndSetProductPrice(value.product, props.customer, props.order, localFixedData.value.requestedQuantity, localFixedData, fixedFormKey)
            return
        }
        if (value.component) {
            await measure.getAndSetComponentPrice(value.component, props.customer, props.order, localFixedData.value.requestedQuantity, localFixedData, fixedFormKey)
        }
    }
}
