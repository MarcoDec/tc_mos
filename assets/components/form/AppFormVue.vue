...
<template>
    <form v-bind="attrs" @submit="onSubmit">
        <template v-if="props.noContent">
            <slot v-if="typeof $slots.default === 'function'" :disabled="props.disabled" :form="props.id" :submit-label="props.submitLabel" type="submit"/>
        </template>
        <template v-for="field in props.fields" v-else :key="field.name">
            ...
            <template v-if="props.submitLabel !== null">
                <div class="row">
                    <div class="col d-inline-flex justify-content-end">
                        <slot v-if="typeof $slots.default === 'function'" :disabled="props.disabled" :form="props.id" type="submit"/>
                        <template v-else>
                            <AppBtnJS :disabled="props.disabled" :form="props.id" type="submit">
                                {{ props.submitLabel }}
                            </AppBtnJS>
                        </template>
                    </div>
